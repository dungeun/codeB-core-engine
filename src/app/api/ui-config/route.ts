...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.log(`[UI Config] Found ${adminMenus.length} admin header menus in database`);
      ...
      for (const menu of adminMenus) {
        ...
        if (label.includes('.') && !label.includes('http')) {
          label = await getTranslation(label, language);
        }
        ...
        console.log(`[UI Config] Added admin menu: ${label} (${content.href}) [order: ${menu.order}]`);
      }
      console.log(`[UI Config] Total admin menus processed: ${adminHeaderMenus.length}`);
    } catch (error) {
    ...
    const headerCategoryMenus = categoryMenus.slice(0, 3).map((cat, index) => ({
      ...
      label: cat.name,
      href: cat.href,
      order: 100 + index, // Admin 메뉴 뒤에 배치
      visible: true
    }));
    ...
    console.log(`[UI Config] Final header menus count: ${allHeaderMenus.length}`);
    console.log('[UI Config] Final header menus:', allHeaderMenus.map(m => `${m.label} (${m.href}) [order: ${m.order}]`));
    ...
    try {
      ...
      if (uiConfig) {
        ...
        console.log('[UI Config] Found saved config in database, merging with admin menus...');
        console.log(`[UI Config] Original saved config header menus: ${savedConfig.header?.menus?.length || 0}`);
        
        // 저장된 설정의 헤더 메뉴를 Admin 메뉴로 교체
        if (savedConfig.header) {
          savedConfig.header.menus = allHeaderMenus;
          console.log(`[UI Config] Updated saved config with ${allHeaderMenus.length} header menus`);
          console.log('[UI Config] Final merged menus:', savedConfig.header.menus.map(m => `${m.label} (${m.href})`));
        }
        
        console.log('[UI Config] Returning merged saved config');
    ...
    // 기본 설정 반환
    console.log('[UI Config] No saved config found, returning default config');
    console.log(`[UI Config] Default config header menus: ${defaultConfig.header.menus.length}`);

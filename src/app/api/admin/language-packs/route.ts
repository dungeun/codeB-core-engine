...
export async function POST(request: NextRequest) {
  try {
    ...
    const { key, ko, en, jp, category, description, autoTranslate } = body;
    ...
    if (autoTranslate && process.env.GOOGLE_TRANSLATE_API_KEY) {
      try {
        ...
        if (!jp) {
          const jpResult = await translationService.translateText(ko, { from: 'ko', to: 'ja' });
          translatedJp = jpResult.translatedText;
        }
      } catch (error) {
    ...
    const languagePack = await prisma.languagePack.create({
      data: {
        ...
        jp: translatedJp || ko, // 번역 실패 시 한국어 사용
        category,
        description,
        isEditable: true
      }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const body = await request.json();
    const { id, ko, en, jp, description } = body;
